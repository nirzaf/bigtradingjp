  ...
  if (!vehicle) {
    return (
      <div className="container-custom py-16 text-center">
        <h2 className="text-2xl font-display font-bold mb-4">{t('vehicle.vehicleNotFound')}</h2>
        <p className="mb-6">{t('vehicle.vehicleNotFoundDesc')}</p>
        <button
          onClick={() => navigate('/')}
          className="btn btn-primary"
        >
          {t('vehicle.returnToHome')}
        </button>
      </div>
    );
  }
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center">
            <div>
              ...
              <p className="text-accent-400 text-lg md:text-xl font-semibold">
                {t('vehicle.contactForPricing')}
              </p>
            </div>
            <div className="mt-4 md:mt-0">
              <a
                href={getWhatsAppUrl(`${t('vehicle.interestedInVehicle')} ${vehicle.make} ${vehicle.model}. ${t('vehicle.moreInfo')}`)}
                target="_blank"
              ...
              <div className="sticky top-24">
                ...
                <div className="bg-gray-50 rounded-lg p-6">
                  <h3 className="text-xl font-display font-semibold mb-4 flex items-center">
                    ...
                    {t('vehicle.performanceSpecs')}
                  </h3>

                  <div className="space-y-4">
                    <div className="flex justify-between py-2 border-b border-gray-200">
                      <div className="text-secondary-700">{t('vehicle.engine')}</div>
                      <div className="font-medium text-right">{vehicle.specs.engine}</div>
                    </div>

                    <div className="flex justify-between py-2 border-b border-gray-200">
                      <div className="text-secondary-700">{t('vehicle.power')}</div>
                      <div className="font-medium">{vehicle.specs.power}</div>
                    </div>

                    <div className="flex justify-between py-2 border-b border-gray-200">
                      <div className="text-secondary-700">{t('vehicle.acceleration')}</div>
                      <div className="font-medium">{vehicle.specs.acceleration}</div>
                    </div>

                    <div className="flex justify-between py-2 border-b border-gray-200">
                      <div className="text-secondary-700">{t('vehicle.topSpeed')}</div>
